...
namespace ERPWebAPI.Controllers.OHS
{
    ...
    {
        readonly IOHS_TrainingTopicService<OHS_TrainingTopic, SqlResult> _trainingTopicService;

        public TrainingTopicController(IOHS_TrainingTopicService<OHS_TrainingTopic, SqlResult> trainingTopicService)
        {
        ...
        {
            var result = _trainingTopicService.GetAllDataMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
        ...
        {
            var result = _trainingTopicService.ResultOperationsMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
                return Ok(result.Data);
            }
