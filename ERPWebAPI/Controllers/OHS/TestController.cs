...
namespace ERPWebAPI.Controllers.OHS
{
    ...
    {
        readonly IOHS_TestService<OHS_Test, SqlResult> _testService;

        public TestController(IOHS_TestService<OHS_Test, SqlResult> testService)
        {
        ...
        {
            var result = _testService.GetAllDataMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
        ...
        {
            var result = _testService.ResultOperationsMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
                return Ok(result.Data);
            }
