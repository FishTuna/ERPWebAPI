...
namespace ERPWebAPI.Controllers.OHS
{
    ...
    public class MmpiResultController : ControllerBase
    {
        ...
        {
            var result = _mmpiResultService.GetAllDataMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
        ...
        {
            var result = _mmpiResultService.ResultOperationsMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
                return Ok(result.Data);
            }
