...
namespace ERPWebAPI.Controllers.OHS
{
    ...
    public class PersonWorkMonitoringListController : ControllerBase
    {
        ...
        {
            var result = _personWorkMonitoringListService.GetAllDataMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
        ...
        {
            var result = _personWorkMonitoringListService.ResultOperationsMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
                return Ok(result.Data);
            }
