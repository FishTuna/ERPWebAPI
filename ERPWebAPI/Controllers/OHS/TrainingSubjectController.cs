...
namespace ERPWebAPI.Controllers.OHS
{
    ...
    {
        readonly IOHS_TrainingSubjectService<OHS_TrainingSubject, SqlResult> _trainingSubjectService;

        public TrainingSubjectController(IOHS_TrainingSubjectService<OHS_TrainingSubject, SqlResult> trainingSubjectService)
        {
        ...
        {
            var result = _trainingSubjectService.GetAllDataMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
        ...
        {
            var result = _trainingSubjectService.ResultOperationsMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
                return Ok(result.Data);
            }
