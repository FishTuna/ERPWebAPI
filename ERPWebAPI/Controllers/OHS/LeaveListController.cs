...
namespace ERPWebAPI.Controllers.OHS
{
    ...
    {
        readonly IOHS_LeaveListService<OHS_LeaveList, SqlResult> _leaveListService;

        public LeaveListController(IOHS_LeaveListService<OHS_LeaveList, SqlResult> leaveListService)
        {
        ...
        {
            var result = _leaveListService.GetAllDataMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
        ...
        {
            var result = _leaveListService.ResultOperationsMngr(module, target, point, parameters);
            if (result.IsSuccess)
            {
                return Ok(result.Data);
            }
